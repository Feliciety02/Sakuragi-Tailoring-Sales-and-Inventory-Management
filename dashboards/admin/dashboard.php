...
$topServices = $pdo->query("
    SELECT s.service_name, 0 as total_orders
    FROM services s
      ...
      <div class="low-stock">
        ...
        <ul>
          <?php foreach ($lowStockItems as $item): ?>
            <li><?= htmlspecialchars($item['item_name']) ?> — <strong><?= $item['quantity'] ?></strong> left (Reorder at: <?= $item['reorder_level'] ?>) <span style="color:red;">🔴</span></li>
          <?php endforeach; ?>
        </ul>
      </div>
          ...
          <?php foreach ($orderTimelines as $row): ?>
            <li>
              <div class="timeline-header">
                <strong>#<?= $row['order_id'] ?> - <?= htmlspecialchars($row['full_name']) ?></strong>
