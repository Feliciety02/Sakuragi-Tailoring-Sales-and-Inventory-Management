...
if (!$lot_inspection_id) { header('Location: aql_dashboard.php'); exit; }

$stmt = $pdo->prepare("SELECT li.*, o.order_id, o.total_price, o.order_date, u.full_name AS customer_name, (SELECT SUM(quantity) FROM order_details WHERE order_id = o.order_id) AS total_qty, ow.assigned_employee FROM qc_lot_inspections li JOIN orders o ON li.order_id = o.order_id JOIN order_workflow ow ON li.workflow_id = ow.workflow_id JOIN users u ON o.user_id = u.user_id WHERE li.lot_inspection_id = ?");
$stmt->execute([$lot_inspection_id]);
$lot = $stmt->fetch();
if (!$lot) { header('Location: aql_dashboard.php'); exit; }
...
$header = renderPageHeader('AQL Lot Inspection #' . $lot_inspection_id, 'Order #' . $lot['order_id'] . ' | ' . htmlspecialchars($lot['customer_name']), '', [['label' => 'Back to Dashboard', 'href' => 'aql_dashboard.php', 'icon' => 'fas fa-arrow-left', 'variant' => 'outline']]);
...
<div style="font-size:0.7rem;color:var(--text-tertiary);margin-top:4px">Zero tolerance</div>
</div>
<div style="text-align:center;background:var(--bg-secondary);border-radius:12px;padding:20px;border:1px solid var(--border-color)">
...
<div style="font-size:0.7rem;color:var(--text-tertiary);margin-top:4px">Ac: <?= $acRe[0] ?>, Re: <?= $acRe[1] ?></div>
</div>
<div style="text-align:center;background:var(--bg-secondary);border-radius:12px;padding:20px;border:1px solid var(--border-color)">
...
<div style="font-size:0.7rem;color:var(--text-tertiary);margin-top:4px">Looser threshold (AQL 4.0)</div>
</div>
</div>
<div style="margin-bottom:16px">
<label style="display:block;font-size:0.8125rem;font-weight:600;color:var(--text-primary);margin-bottom:6px">Notes</label>
<textarea name="notes" rows="3" placeholder="Describe defects found..." style="width:100%;padding:10px 12px;border:1px solid var(--border-color);border-radius:8px;font-size:0.85rem;resize:vertical;background:var(--bg-primary);color:var(--text-primary);font-family:inherit"></textarea>
</div>
<div id="verdictPreview" class="verdict-box verdict-pending" style="margin-bottom:16px">
    ...
    <p style="margin:0" id="verdictText">Adjust defect counts to see verdict.</p>
</div>
<div style="display:flex;gap:8px;justify-content:flex-end;padding-top:16px;border-top:1px solid var(--border-color)">
    <a href="aql_dashboard.php" class="dash-btn dash-btn-outline dash-btn-sm"><i class="fas fa-times"></i> Cancel</a>
    <button type="submit" name="submit_inspection" class="dash-btn dash-btn-primary dash-btn-sm"><i class="fas fa-check"></i> Submit Inspection</button>
</div>
</form>
<?php else:
...
<div class="verdict-box <?= $vc ?>" style="margin-bottom:16px">
    ...
    <p style="margin:0 0 4px">Crit: <?= $lot['critical_defects'] ?> | Maj: <?= $lot['major_defects'] ?> | Min: <?= $lot['minor_defects'] ?></p>
    <?php if ($lot['notes']): ?><p style="margin:0;font-size:0.85rem"><?= htmlspecialchars($lot['notes']) ?></p><?php endif; ?>
</div>
<?php endif;
...
function updateVerdict() {
    ...
    else if (major >= majorReject) { verdict = \'Failed\'; cls = \'verdict-fail\'; icon = \'fa-times-circle\'; text.innerText = \'Failed: \' + major + \' major defects exceeds reject of \' + majorReject + \'.\'; }
    else if (minor >= minorReject) { verdict = \'Failed\'; cls = \'verdict-fail\'; icon = \'fa-times-circle\'; text.innerText = \'Failed: \' + minor + \' minor defects exceeds reject of \' + minorReject + \'.\'; }
    else { verdict = \'Passed\'; cls = \'verdict-pass\'; icon = \'fa-check-circle\'; text.innerText = \'Passed: critical \' + critical + \' major \' + major + \'/\' + majorAccept + \' minor \' + minor + \'/\' + minorAccept + \'.\'; }
